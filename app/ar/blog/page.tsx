  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-800 py-12 px-4 sm:px-6 lg:px-8 font-tajawal" dir="rtl">
      <div className="max-w-4xl mx-auto">
        ...
        {/* هيدر الصفحة المحسن دلالياً لمحركات البحث */}
        ...
        {/* شبكة عرض الأدلة والمقالات المؤرشفة */}
        <div className="grid gap-8">
          {arabicArticles.map((article) => (
            <article key={article.slug} className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-2xs hover:shadow-xs transition-shadow">
              ...
              {/* تصنيف دلالي مستهدف غني بالكلمات المفتاحية للأدوات المدعومة */}
              <span className="inline-block bg-emerald-50 text-emerald-700 text-xs font-bold px-2.5 py-1 rounded-md mb-3 font-tajawal">
                أداة {article.targetTool}
              </span>
              
              {/* عنوان الرابط المباشر - الأقوى لتمرير عصير الروابط الداخلي Link Juice */}
              <h2 className="text-xl sm:text-2xl font-bold text-slate-900 hover:text-emerald-600 transition-colors font-tajawal">
                <Link href={`/ar/blog/${article.slug}`}>{article.title}</Link>
              </h2>
              
              {/* مقتطف المقال المقتضب الغني بالكلمات المفتاحية الطويلة */}
              <p className="text-slate-600 mt-2 text-sm sm:text-base line-clamp-3 font-tajawal">
                {article.excerpt}
              </p>
              
              {/* شريط البيانات الوصفية السفلي */}
              <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-100 text-xs text-slate-400 font-mono">
                ...
                <Link href={`/ar/blog/${article.slug}`} className="text-emerald-600 font-bold hover:underline font-tajawal">
                  قراءة الدليل التقني ←
                </Link>
              </div>
